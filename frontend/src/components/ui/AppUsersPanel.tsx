...
export default function AppUsersPanel({ appName, onClose }: { appName: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-40 flex">
      ...
      <div className="relative ml-auto w-[480px] bg-white shadow-2xl flex flex-col h-full">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gradient-to-r from-indigo-50 to-purple-50">
          <div>
            <p className="text-xs text-gray-500 mb-0.5">Приложение</p>
            <h2 className="text-base font-semibold text-gray-900 break-all">{appName}</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          ...
          {entries.map((e, i) => (
            <div key={i} className="bg-gray-50 rounded-xl p-4 space-y-2.5">
              {/* User */}
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold text-gray-900">{fioOf(e) ?? e.username}</p>
                  {fioOf(e) && <p className="text-xs text-gray-400">{e.username}</p>}
                </div>
                <span className="ml-auto text-xs text-gray-400">{e.launch_count} запусков</span>
              </div>
              ...
              {e.full_path ? (
                <div className="flex items-start gap-2 text-xs text-gray-500">
                  ...
                  <span className="break-all font-mono bg-gray-100 rounded px-2 py-1 leading-relaxed">{e.full_path}</span>
                </div>
              ) : (
